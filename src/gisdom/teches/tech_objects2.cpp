...
#include "gcprec.h"
#include "tech_objects2.h"


void gceTechObjects2::init()
{
    //wxStopWatch sw;
    m_progBasic = m_storage.progs.FindOrCreate("objects2");
    if (!m_progBasic->IsOk())
    {
        gl::ProgramInfo info;
        info.addShaderFile(GL_VERTEX_SHADER, "prog/objects2.vert");
        info.addShaderFile(GL_FRAGMENT_SHADER, "prog/objects2.frag");
        if (!m_progBasic->Create(info))
        {
            gceContext::log_error("prog/objects2 load error");
        }
    }
...
void gceTechObjects2::prepareScene(gceContext &ctx, const glm::dmat4 &proj, const geom::aabb2 &aoi, const glm::ivec2 &res)
{
    ...
    collectBatches(proj, aoi);
}

void gceTechObjects2::collectBatches(const glm::dmat4 &proj, const geom::aabb2 &)
{
    m_batches.clear();
...
void gceTechObjects2::renderScene(const glm::dmat4 &proj, const geom::aabb2 &aoi, const glm::ivec2 &res)
{
    //glEnable(GL_DEPTH_TEST);

    m_progBasic->Begin();

    const float scale = aoi.size.y / 30.0;
    m_progBasic->setValue(loc_scale, static_cast<float>(scale));
    ...
    for (auto &b : m_batches)
    {
        ...
        glBindVertexBuffer(0, b.array, 0, sizeof(tech_objects2::ArrayVertex));
...
void gceTechObjects2::processMsg(const urenderTechDataMsg &msg)
{
    if (msg.id_model == id_model && msg.techType == gceTechType::BASIC)
    {
        processData(static_cast<const tech_objects2::DataMsg &>(msg));
    }
}
//#include <fmt/ranges.h>
void gceTechObjects2::processData(const tech_objects2::DataMsg &msg)
{
    if (msg.clearArrays)
    {
        m_arrays.clear();
...
void gceTechObjects2::setArray(const tech_objects2::ArrayInfo &info)
{
    glNamedBufferData(m_arrays[info.id_array].name(), sizeof(tech_objects2::ArrayVertex) * info.vertexes.size(), info.vertexes.data(), GL_STATIC_DRAW);
}

void gceTechObjects2::setIndex(const tech_objects2::IndexInfo &info)
{
    glNamedBufferData(m_indexes[info.id_index].name(), sizeof(tech_objects2::IndexInfo::index_type) * info.indexes.size(), info.indexes.data(), GL_STATIC_DRAW);
}

void gceTechObjects2::setMesh(const tech_objects2::MeshInfo &info)
{
    using Vertex = tech_objects2::ArrayVertex;
...
void gceTechObjects2::setInstance(const tech_objects2::InstanceInfo &info)
{
...
void gceTechObjects2::setVAO()
{
    glVertexArrayAttribFormat(m_vao.name(), loc_position, 3, GL_FLOAT, GL_FALSE, offsetof(tech_objects2::ArrayVertex, pos));
